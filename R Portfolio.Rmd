...
Learning objective(s) assessed: **LO2 | LO4**
...
shape_var <- 1/shape_trigam

z_score <- 1.96  # Approximate standardised value for 95% conf int

shape_lwrb <- fit_gamma$estimate['shape'] - z_score * sqrt(shape_var)
shape_uprb <- fit_gamma$estimate['shape'] + z_score * sqrt(shape_var)

rate_fisher <- fit_gamma$estimate['shape'] / (fit_gamma$estimate['rate']^2)

rate_var <- 1/rate_fisher

rate_lwrb <- fit_gamma$estimate['rate'] - z_score * sqrt(rate_var)
rate_uprb <- fit_gamma$estimate['rate'] + z_score * sqrt(rate_var)

voltage_mean <- mean(dat_lv$Value)

ci_table <- data.frame(
  Confidence_Intervals = c("2.5%", "97.5%"),
